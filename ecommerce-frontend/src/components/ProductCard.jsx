...
const ProductCard = ({ product }) => {
  ...
  const discount = hasPromo && Math.round(((product.oldPrice - product.price) / product.oldPrice) * 100);
  ...
  return (
    <div onClick={goToDetail} className="group cursor-pointer flex flex-col w-full max-w-[280px] mx-auto">
      <div className="relative aspect-[3/4] rounded-[2rem] bg-white border border-gray-200 hover:border-gray-300 transition-colors duration-300 overflow-hidden mb-4">
        ...
        <button onClick={handleCartAction} className={`md:hidden absolute bottom-3 right-3 w-10 h-10 rounded-full flex items-center justify-center shadow-lg active:scale-90 transition-transform ${isInCart ? "bg-red-500 text-white" : "bg-black text-white"}`}>
          {isInCart ? <Trash2 size={18} /> : <Plus size={20} />}
        </button>
      </div>
      <div className="px-1">
        {product.brand && <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">{product.brand}</p>}
        <h3 className="text-sm font-medium text-gray-900 leading-snug line-clamp-2">{product.name}</h3>
        <div className="flex items-center gap-3 mt-2">
          <span className="text-base font-bold text-gray-900">{formatPrice(product.price)}</span>
          {hasPromo && <span className="text-xs text-gray-500 line-through decoration-red-500/30">{formatPrice(product.oldPrice)}</span>}
        </div>
      </div>
    </div>
  );
